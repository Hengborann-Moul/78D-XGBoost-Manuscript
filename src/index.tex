...
\begin{center}
  ...
  {\fontsize{12}{22}
    ...
    Sothea HAS\textsuperscript{\color{superscript}\hyperlink{aff2}{\color{superscript} 2}},\;
    Sokkhey PHAUK\textsuperscript{\color{superscript}\hyperlink{aff2}{\color{superscript} 2}}\\[6pt]
  }
...
\begin{multicols}{2}
  ...
  \section{Methodology}
  \subsection{Dataset}
  \subsubsection{DAiSEE}

  This study employs the DAiSEE (Dataset for Affective States in E-Environments) dataset, a large-scale benchmark for affective state recognition in online learning contexts\cite{Gupta2016DAiSEETU}. The dataset comprises video recordings of students engaged with online educational content. The DAiSEE dataset provides annotations for four distinct affective states: boredom, engagement, confusion, and frustration. Each affective state is annotated using a four-level ordinal scale (Very Low = 0, Low = 1, High = 2, Very High = 3) based on crowd-sourced annotations from multiple raters as illustrated in Table~\ref{tab_daisee_labels} below.

  %Figure~\ref{fig:daisee_participants} shows representative frames from the DAiSEE dataset, illustrating the naturalistic, in-the-wild recording conditions and the diversity of participants.
  ...
  \begin{figure}[H]
    ...
    \begin{subfigure}[b]{\columnwidth}
      \centering
      \includegraphics[width=\columnwidth]{image/bored0-3.png}
    \end{subfigure}
    \begin{subfigure}[b]{\columnwidth}
      \centering
      \includegraphics[width=\columnwidth]{image/engaged0-3.png}
    \end{subfigure}
    % \begin{subfigure}[b]{\columnwidth}
    %   \centering
    %   \includegraphics[width=\columnwidth]{image/conf0-3.png}
    % \end{subfigure}
    % \begin{subfigure}[b]{\columnwidth}
    %   \centering
    %   \includegraphics[width=\columnwidth]{image/frus0-3.png}
    % \end{subfigure}
    \caption{Sample participant frames from the DAiSEE dataset, showing naturalistic expressions of affective states during online learning sessions. Images are captured under varied lighting conditions, camera angles, and personal environments typical of in-the-wild educational settings.}
    \label{fig:daisee_participants}
  \end{figure}
  \vspace{-1cm}
  ...
  \subsection{Data Preprocessing: MediaPipe Feature Extraction}
  ...
  Picking the right metrics matters as much as picking the right model. Raw accuracy looks clean on paper, but DAiSEE is heavily imbalanced. Engagement samples outnumber Boredom and Confusion by a wide margin, so a classifier that favors the dominant class can post a high accuracy score while completely missing the states that matter most in a real classroom. Weighted F1-score serves as the primary indicator here, supported by per-class precision, recall, and F1-score for a finer view of each affective state.
  ...
  This section evaluates the proposed XGBoost-based framework for affective state recognition on the DAiSEE dataset. We report Accuracy, Precision, Recall, and F1-score for each of the four target states: boredom, engagement, confusion, and frustration. Metrics are computed per class and averaged using both marco and weighted strategies, which is necessary given the pronounced class imbalance in DAiSEE annotations.
  ...
  \midrule
  Boredom     & 0.478 & 0.465 & 0.516 & 0.528 \\
  Engagement  & 0.478 & 0.424 & 0.596 & 0.603 \\
  Confusion   & 0.520 & 0.338 & 0.555 & 0.675 \\
  Frustration & 0.521 & 0.336 & 0.716 & 0.802 \\
  \midrule
  \textbf{Mean} & \textbf{0.499} & \textbf{0.391} & \textbf{0.596} & \textbf{0.652} \\
  \bottomrule
  \end{tabular}
  \end{table}
  ...
  Figure~\ref{fig:confusion-matrices} breaks these aggregates down into per-class recall patterns. The diagonal entries show the proportion of each true class that the model correctly identifies. %These per-class patterns reveal a consistent trade-off. The model achieves respectable accuracy when the majority class is large and visually distinctive as in frustration, where facial tension produces reliable cues, but fails when classes are rare or expressions are subtle. This motivates the broader comparison against existing methods to gauge whether the observed limitations are specific to this pipeline or endemic to the dataset itself.

  \begin{figure*}
  \centering
  \begin{tikzpicture}
    \begin{groupplot}[
        group style={group size=4 by 1, horizontal sep=0.45cm},
        width=0.19\textwidth,
      ...
      \nextgroupplot[title={Boredom}, ylabel={True},]
      \addplot [
        matrix plot*,
        mesh/cols=3,
        point meta=explicit,
      ...
      \nextgroupplot[title={Engagement}, yticklabels={,,}]
      \addplot [
        matrix plot*,
        mesh/cols=3,
        point meta=explicit,
      ...
      \nextgroupplot[ title={Confusion}, yticklabels={,,} ]
      \addplot [
        matrix plot*,
        mesh/cols=3,
        point meta=explicit,
        ...
        nodes near coords={%
            \ifnum\coordindex=0
            \textcolor{white}{\pgfmathprintnumber[fixed,fixed zerofill,precision=2]{\pgfplotspointmeta}}%
            \else\ifnum\coordindex=3
            \textcolor{white}{\pgfmathprintnumber[fixed,fixed zerofill,precision=2]{\pgfplotspointmeta}}%
            \else\ifnum\coordindex=6
            \textcolor{white}{\pgfmathprintnumber[fixed,fixed zerofill,precision=2]{\pgfplotspointmeta}}%
            \else
      ...
      \nextgroupplot[title={Frustration}, yticklabels={,,}]
      \addplot [
        matrix plot*,
        mesh/cols=3,
        point meta=explicit,
  ...
  %Rows correspond to true labels and columns to predicted labels. Diagonal values indicate per-class recall.}
  \label{fig:confusion-matrices}
  \end{figure*}

  \begin{comment}
  \textbf{Boredom} shows the most balanced confusion matrix, yet remains the weakest state overall. The model recalls only 63\% of true Low boredom samples, with the remainder split between Medium (27\%) and High (10\%). True Medium and High boredom are even more entangled: Medium samples are misclassified as Low 41\% of the time, while High samples are correctly recalled at just 45\% with 29\% drifting to Medium. This pattern confirms that boredom lacks distinctive facial activations; its signature is often the absence of strong expression, making it easy to confuse with neutral or low-intensity states.

  \textbf{Engagement} presents a different failure mode. The model almost entirely misses the Low engagement minority class, recalling only 11\% of true Low samples while misclassifying 76\% of them as Medium. True Medium engagement fares better at 65\% recall, and High engagement reaches 53\%. The near-total collapse on Low engagement---despite focal loss and calibration---suggests that the facial cues distinguishing low engagement from medium engagement are subtle and underrepresented in the training data.

  \textbf{Confusion} and \textbf{Frustration} both suffer from severe majority-class bias, though to different degrees. For confusion, the model correctly recalls 92\% of true Low samples but only 13\% of Medium and 6\% of High, with the vast majority of minority-class instances collapsing into the Low bucket. Frustration shows the same trend in its most extreme form: 97\% recall for Low, 4\% for Medium, and 5\% for High. The near-diagonal dominance in the first column of both matrices reveals that the classifier has learned a safe but uninformative strategy: predict Low unless the evidence is overwhelming. This behavior is consistent with the DAiSEE label distribution, where Low-intensity annotations dominate all four states.
  ...
  Table~\ref{tab:daisee-sota} places our results alongside existing methods on the DAiSEE dataset. We report two figures for our model: a mean accuracy of 65.26\% across all four affective states, and an engagement-specific accuracy of 60.30\%.
  One comparison challenge worth flagging: most prior work in this space targets engagement detection alone, reporting a single-task accuracy figure. Our framework does something different. It classifies all four states simultaneously, covering boredom, engagement, confusion, and frustration in a single pass. Direct numerical comparisons should be read with that scope difference in mind.
  It is also worth noting what those prior methods cost to run. The leading baselines lean on architectures built around 3D convolutions, dual-stream EfficientNet backbones, and LSTM or Transformer temporal modules. These are not lightweight choices. They demand substantial GPU resources at both training and inference time, which limits where and how they can realistically be deployed.
  ...
  \label{tab:daisee-sota-old}
  \begin{tabular}{>{\small}l>{\small}c}
  \toprule
  \textbf{Model \cite{Gothwal2025ViBEDNetVB}} & \textbf{Accuracy (\%)} \\
  \midrule
  CNN-RNN Baseline (DAiSEE) \cite{Gupta2016DAiSEETU} & 53.90 \\
  I3D (Inflated 3D ConvNet) \cite{Zhang2019AnNE} & 52.35 \\
  DFSTN (SE-ResNet-50 + LSTM + GALN) \cite{Liao2021DeepFS} & 58.84 \\
  ResNet + TCN Hybrid \cite{Abedi2021ImprovingSI} & 63.90 \\
  EfficientNetB7 + LSTM \cite{Selim2022StudentsEL} & 67.48 \\
  BiusFPN + ICCSA \cite{Naveen2025StudentEA} & 68.16 \\
  General Model for Learner Engagement \cite{Malekshahi2024AGM} & 68.57 \\
  Affect-driven Ordinal Engagement \cite{Abedi2021AffectdrivenOE} & 67.40 \\
  VisioPhysioENet \cite{Singh2024VisioPhysioENetME} & 63.09 \\
  ViBED-Net (Transformer) \cite{Gothwal2025ViBEDNetVB} & 62.39 \\
  ViBED-Net (LSTM) \cite{Gothwal2025ViBEDNetVB} & 73.43 \\
  \midrule
  \textbf{Proposed XGBoost } & \textbf{65.26} \\
  \bottomrule
  \end{tabular}
  \end{table}
  \end{comment}
  ...
  \begin{table}[H]
  \centering
  \caption{Accuracy comparison with existing approaches on the DAiSEE dataset. Results for prior methods are engagement-only (single-task), whereas our XGBoost model performs multi-task classification across four affective states\cite{Gothwal2025ViBEDNetVB}.}
  \label{tab:daisee-sota}
  \begin{tabular}{>{\small}l>{\small}c}
  \toprule
  \textbf{Model} & \textbf{Accuracy (\%)} \\
  \midrule
  CNN-RNN Baseline (DAiSEE) & 53.90 \\
  I3D (Inflated 3D ConvNet) & 52.35 \\
  DFSTN (SE-ResNet-50 + LSTM + GALN) & 58.84 \\
  ResNet + TCN Hybrid & 63.90 \\
  EfficientNetB7 + LSTM & 67.48 \\
  BiusFPN + ICCSA & 68.16 \\
  General Model for Learner Engagement & 68.57 \\
  Affect-driven Ordinal Engagement & 67.40 \\
  VisioPhysioENet & 63.09 \\
  ViBED-Net (Transformer) & 62.39 \\
  ViBED-Net (LSTM) & 73.43 \\
  \midrule
  \textbf{Proposed XGBoost } & \textbf{65.26} \\
  \bottomrule
  \end{tabular}
  \end{table}
  \vspace{-0.5cm}
  While the XGBoost model does not surpass the state-of-the-art ViBED-Net (LSTM) engagement accuracy of 73.43\%, it achieves a competitive mean multi-task accuracy of 65.26\%, which exceeds several established deep learning baselines including the CNN-RNN baseline (53.90\%), I3D (52.35\%), and the ViBED-Net Transformer variant (62.39\%). Notably, the ResNet + TCN Hybrid (63.90\%) and VisioPhysioENet (63.09\%) achieve engagement-only accuracies comparable to our mean multi-task performance, underscoring that gradient boosting on carefully engineered features can approach the performance of deep spatiotemporal architectures at a fraction of the computational cost.

  The XGBoost pipeline's real advantage is not accuracy. It is the ability to run \textbf{anywhere, on anything}. The full inference path, from MediaPipe feature extraction through to final classification, requires no GPU. On a standard consumer CPU, predtictions arrive in milliseconds per video sequence, and the saved model occupies only a few megabytes. Compare that to ViBED-Net, which stacks dual EfficientNetV2 backbones carrying millions of parameters, adds LSTM or Transformer temporal modules on top, and still needs GPU acceleration to hit real-time throughput. For schools and institutions without dedicated GPU infrastructure, that hardware dependency is a non-starter. The XGBoost approach trades some ceiling performance for something more valuable in practice: the ability to actually deploy.

  The pipeline's modular structure also makes retraining and adaptation straightforward. Because features are engineered explicitly rather than learned end-to-end, there is no black box to reverse-engineer. Educators and researchers can inspect which facial actions drive predictions for each affective state using SHAP values or native XGBoost gain metrics. They can adjust the feature engineering pipeline to handle different cultural expressions or camera setups without touching a neural network. That kind of transparency matters in educational technology. Knowing \textit{why} a model flags disengagement is often just as useful as the flag itself, because it points directly toward what an instructor can do next.
  ...
  \begin{figure*}[t]
    ...
    \begin{subfigure}[b]{0.24\textwidth}
      \centering
      \includegraphics[width=\linewidth]{image/boredom_confusion_matrix.png}
      \caption{Boredom}
    ...
    \begin{subfigure}[b]{0.24\textwidth}
      \centering
      \includegraphics[width=\linewidth]{image/engagement_confusion_matrix.png}
      \caption{Engagement}
    ...
    \begin{subfigure}[b]{0.24\textwidth}
      \centering
      \includegraphics[width=\linewidth]{image/confusion_confusion_matrix.png}
      \caption{Confusion}
    ...
    \begin{subfigure}[b]{0.24\textwidth}
      \centering
      \includegraphics[width=\linewidth]{image/frustration_confusion_matrix.png}
      \caption{Frustration}
      \label{subfig:frustration-cm}
    \end{subfigure}
    \caption{Confusion matrices for XGBoost model predictions across four affective states on the test set.}
    \label{fig:confusion-matrices-old}
  \end{figure*}
  \end{comment}
